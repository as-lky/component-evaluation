...
def SCIP_solver(n, m, k, site, value, constraint, constraint_type, coefficient, time_limit, obj_type, now_sol, now_col, constr_flag, lower_bound, upper_bound, value_type):
    '''
    函数说明：
    ...
    for i in range(n):
        if(now_col[i] == 1):
            ...
            if(value_type[i] == 'B'):
                x[new_num] = model.addVar(lb = lower_bound[i], ub = upper_bound[i], vtype = "B")
            elif(value_type[i] == 'C'):
                x[new_num] = model.addVar(lb = lower_bound[i], ub = upper_bound[i], vtype = "C")
            else:
                x[new_num] = model.addVar(lb = lower_bound[i], ub = upper_bound[i], vtype = "I")
            new_num += 1
    #定义new_num个决策变量x[]
...
#multilevel_FENNEL
def multilevel_FENNEL(partition_var, partition_num, n, m, k, rate, site):
    ...
    for i in range(partition_num):
        ...
        if(now_num[now_color] + len(partition_var[now_site]) >= int(rate * n) - 1):
            now_color += 1
        if(now_color >= 4):
            break
    ...
    '''
    while(now_color < 4):
        now_cons = random.randint(0, m - 1)
        if(now_num[now_color] + k[now_cons] >= int(rate * n) - 1):
            now_color += 1
        if(now_color >= 4):
            break
        for j in range(k[now_cons]):
            var = site[now_cons][j]
            if(parts[now_color][var] == 0):
                parts[now_color][var] = 1
                now_num[now_color] += 1
    '''
    return(parts)
...
def cross(n, m, k, site, value, constraint, constraint_type, coefficient, obj_type, rate, solA, blockA, solB, blockB, set_time, constr_flag, lower_bound, upper_bound, value_type):
    ...
    if(add_num / n <= rate):
        newcrossX, newVal = SCIP_solver(n, m, k, site, value, constraint, constraint_type, coefficient, set_time, obj_type, crossX, color, constr_flag, lower_bound, upper_bound, value_type)
...
# Load data
now_num = 4
if(os.path.exists('./example-MVC-h/pair' + str(now_num) + '.pickle') == False):
    print("No problem file!")

with open('./example-MVC-h/pair' + str(now_num) + '.pickle', "rb") as f:
...
if(os.path.exists('./example-MVC-h/data' + str(now_num) + '.pickle') == False):
    print("No problem file!")

with open('./example-MVC-h/data' + str(now_num) + '.pickle', "rb") as f:
    problem = pickle.load(f)
...
path_model = "./90.pkl"
model = SpGAT(nfeat=features.shape[1],    #特征维度
            nhid=64,                      #隐藏层每一层的特征维度
            nclass=2,                     #类别数
            dropout=0.5,                  #dropout
            nheads=6,                 #头数
            alpha=0.2)                    #LeakyReLU alpha系数
...
for turn in range(20):
            ...
            if(constr + flag_B > constraint[i]):
                ...
                for j in range(k[i]):
                    if(color[site[i][j]] == 0):
                        ...
                        if(constr + flag_B <= constraint[i]):
                            break
            elif(number > 0 and color[site[i][0]] == 0 and solution[site[i][0]] == 0):
                color[site[i][0]] = 1
                obj -= 1
            ...
            if(constr + flag_A < constraint[i]):
                ...
                for j in range(k[i]):
                    if(color[site[i][j]] == 0):
                        ...
                        if(constr + flag_A >= constraint[i]):
                            break
            elif(constr + flag_B > constraint[i]):
                for j in range(k[i]):
                    if(solution[site[i][j]] == 1):
                        color[site[i][j]] = 0
                        constr -= solution[site[i][j]] * value[i][j]
                        if(value[i][j] >= 0):
                            flag_A += upper_bound[site[i][j]] * value[i][j]
    ...
    if(obj / n + rate >= 1):
        ...
        set_rate -= 0.05
nowX, nowVal = SCIP_solver(n, m, k, site, value, constraint, constraint_type, coefficient, 0.5 * set_time, obj_type, solution, color, constr_flag, lower_bound, upper_bound, value_type)
...
while(time.time() - begin_time < set_time):
    if(constr_rate > set_constr_rate):
        #turn += 1
        #if(turn >= 10):
        constr_rate -= 0.3
    ...
    if(constr_rate < 1):
        ...
        if(obj / n + rate < 1):
            print("QwQ")
            exit()
        nowX, nowVal = SCIP_solver(n, m, k, site, value, constraint, constraint_type, coefficient, (0.5 * set_time), obj_type, nowX, color, constr_flag, lower_bound, upper_bound, value_type) 
    ...
    block_list = multilevel_FENNEL(partition_var, partition_num, n, m, k, rate, site)
    print("after:", time.time() - begin_time)
    for i in range(4):
        max_time = set_time - (time.time() - begin_time)
        if(max_time <= 0):
            break
        newX, newVal = SCIP_solver(n, m, k, site, value, constraint, constraint_type, coefficient, max_time, obj_type, nowX, block_list[i], constr_flag, lower_bound, upper_bound, value_type)
        if(newVal != -1):
            turnX.append(newX)
...
if(obj / n + rate < 1):
    print("QwQ")
    exit()
nowX, nowVal = SCIP_solver(n, m, k, site, value, constraint, constraint_type, coefficient, (0.5 * set_time), obj_type, solution, color, constr_all, lower_bound, upper_bound, value_type) 
ans_set.append(nowVal)
time_set.append(time.time() - begin_time)
